...
早期开发版本的 `~/Library/Application Support/Agent Ferry` 不会在进程启动或 Homebrew
`post_install` 中静默搬移。需要保留开发数据时，用户先停止旧 daemon，再显式调用
`aferry data migrate`；双目录并存或符号链接会被拒绝，避免覆盖或越界移动用户数据。

## 组合根

- daemon 组合实时任务运行路径。
- Homebrew 管理程序文件和版本；CLI 组合激活、诊断与清理路径，并通过 `aferry service` 管理当前用户的 macOS LaunchAgent。服务 plist 与日志遵循平台目录，卸载服务不删除用户数据。
